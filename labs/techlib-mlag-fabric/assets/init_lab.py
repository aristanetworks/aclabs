...
Lives at <lab>/assets/init_lab.py — a per-lab artifact shipped alongside
the topology and metadata. Invoked by VS Code tasks.json on folderOpen,
or manually from the shell:

    python3 assets/init_lab.py                 # use current working directory
    python3 assets/init_lab.py --lab-dir PATH  # explicit lab directory
...
def resolve_lab_dir(explicit: Optional[Path] = None) -> Path:
    ...
    init_lab.py lives at <lab>/assets/init_lab.py — it's a per-lab artifact
    shipped alongside the topology and metadata. Discovery uses the current
    working directory (which tasks.json sets to ${workspaceFolder}) or an
    explicit --lab-dir flag.
    ...
    raise FileNotFoundError(
        ...
        f"  python3 assets/init_lab.py --lab-dir /path/to/lab"
    )
...
def build_dashboard_markdown(cfg: LabConfig, total_elapsed: float) -> str:
    ...
    lines.append("")
    lines.append(
        "<sub>Auto-generated by <code>assets/init_lab.py</code> on lab readiness. "
