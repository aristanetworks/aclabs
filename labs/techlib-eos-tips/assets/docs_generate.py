...
Lives at <lab>/assets/docs_generate.py — a per-lab artifact shipped
alongside the topology and metadata. Operates on whatever lab directory
it's invoked from, or use --lab-dir to point at one explicitly.
...
Two marker styles are supported:
  ...
  ...content generated from assets/lab.yml...
...
Usage:
  python3 assets/docs_generate.py                              # rewrite README.md in place
  python3 assets/docs_generate.py --check                      # exit non-zero if README would change (CI)
  python3 assets/docs_generate.py --diff                       # show what would change without writing
  python3 assets/docs_generate.py --lab-dir /path/to/lab       # operate on a specific lab
...
LAB_YML_PATH = Path("assets/lab.yml")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not lab_yml_path.is_file():
        missing.append(f"assets/lab.yml (expected at {lab_yml_path})")
    ...
    if missing:
        ...
        print(
            ...
            "  python3 assets/docs_generate.py --lab-dir /path/to/lab",
            file=sys.stderr,
        )
        return 2
    ...
    if args.check:
        print(
            ...
            f"       Run `python3 assets/docs_generate.py` (or `make docs`) to regenerate.",
            file=sys.stderr,
        )
        return 1
