...
resolution, so the sibling always matches wherever init_lab is running from),
or invoked manually from a lab directory.
...
Usage:
  ...
  python3 docs_generate.py --check                      # exit non-zero if README would change (CI)
  python3 docs_generate.py --diff                       # show what would change without writing
  python3 docs_generate.py --lab-dir /path/to/lab       # operate on a specific lab
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if missing:
        ...
        print(
            "tip: cd into a lab directory and re-run, or pass --lab-dir /path/to/lab",
            file=sys.stderr,
        )
        return 2
    ...
    if args.check:
        print(
            ...
            f"       Run `make docs` (or re-run docs_generate.py) to regenerate.",
            file=sys.stderr,
        )
        return 1
