...
the bundled path first and falls back to the workspace path if the
bundled one is absent. Can also be run manually from a lab directory:

    python3 init_lab.py                 # use current working directory
...
DEFAULT_PER_NODE_TIMEOUT = 1200     # 20 min — generous for cold cEOS + startup-config
...
def resolve_lab_dir(explicit: Optional[Path] = None) -> tuple[Path, Path]:
    ...
    Resolution:
      ...
      2. Otherwise, use the current working directory. When launched
         by the lab-dashboard VS Code extension, cwd is set to the
    ...
    raise FileNotFoundError(
        ...
        f"Missing:\n  - " + "\n  - ".join(missing) + "\n\n"
        f"Tip: cd into a lab directory and re-run, or pass --lab-dir /path/to/lab"
    )


def load_lab_overrides(lab_root: Path) -> dict:
    """Load optional assets/lab.yml if present."""
    override_path = lab_root / "assets" / "lab.yml"
...
SSH_CONFIG_BEGIN_MARK = "# >>> aclabs assets/init_lab.py — managed block (do not edit) >>>"
SSH_CONFIG_END_MARK   = "# <<< aclabs assets/init_lab.py — managed block <<<"
...
async def run(cfg: LabConfig, console: Console) -> int:
    ...
    #
    # Co-located with init_lab.py — Path(__file__).parent resolves to
    # /bin/ when running bundled, or <lab>/assets/ when running from
    # the workspace fallback. Either way, docs_generate.py is the
    # sibling file.
    docs_script = Path(__file__).parent / "docs_generate.py"
